...
COLS = [
    ...
    # ----- 해설 (HTML + <img>) -----
    ('해설',         55, 'exp',   '해설',     'HTML 작성. 그림은 <img src="파일명"> 본문 어디에나 (여러 개·교차 OK)'),
    # ----- 오답분석 (HTML + <img>) -----
    ('오답분석',     45, 'wr',    '오답분석', 'HTML <ol> 권장. 그림 필요 시 <img src="파일명"> 삽입 가능'),
    # ----- 학습 POINT (HTML + <img>) -----
    ('학습포인트',   55, 'lp',    '학습POINT', 'HTML 작성. 그림은 <img src="파일명"> 삽입 (공식·표와 교차 OK)'),
]
...
SAMPLES = [
    {  # 1번 콘덴서 — 단순
        ...
        '정답(1~4)': 4,
        '해설_블록': [
            {'type': 'text', 'value': '정전용량(<i>C</i>)은 극판의 넓이(<i>S</i>)에 비례하고 극판의 간격(<i>d</i>)에 반비례한다. 따라서 ④번의 \'극판의 간격에 비례한다\'는 설명은 틀린 것이다.'},
        ],
        '오답분석': '<ol><li><i>C</i> = <i>Q</i> / <i>V</i> 관계에 의해 전압에 반비례한다.</li><li><i>Q</i> = <i>CV</i> 관계에 의해 전하량에 비례한다.</li><li>정전용량 공식에 의해 넓이에 비례한다.</li></ol>',
        '학습포인트_블록': [
            {'type': 'text', 'value': '<p><strong>정전용량 공식</strong></p><p><i>C</i> = <i>εS</i> / <i>d</i> [F]</p><ul><li><i>C</i>: 정전용량[F]</li><li><i>ε</i>: 유전율[F/m]</li><li><i>S</i>: 극판 단면적[m<sup>2</sup>]</li><li><i>d</i>: 극판 간격[m]</li></ul>'},
        ],
    },
...
GUIDE_ROWS = [
    ...
    ('발문', '문제 본문 (HTML 허용)', '필수', 'μ<sub>s</sub>는 ...'),
    ('조건', '주어진 조건 (없으면 비움)', '선택', 'P=100[Ω]'),
    ('발문그림', '발문에 들어가는 그림 파일명 / [필요: …]', '선택', 'q01_stem.png'),
    ('보기1~4', '4지선다 본문 (HTML 허용)', '필수', '<i>V<sub>l</sub></i> = √3 <i>V<sub>p</sub></i>'),
    ('보기1~4그림', '보기가 그림형일 때 파일명', '선택', 'opt_q35_1.png'),
    ('정답(1~4)', '1·2·3·4 중 하나', '필수', '4'),
    ('해설', 'HTML 그대로 작성. 그림은 <img src="파일명">을 본문 어디에나 삽입 (여러 개·텍스트와 교차 가능)', '필수', '<p>본문</p><img src="q01_a.png"><p>이어서</p><img src="q01_b.png">'),
    ('오답분석', 'HTML 그대로 작성. 그림 필요 시 <img src="파일명"> 삽입 가능 (<ol><li> 권장)', '선택', '<ol><li>…</li></ol><img src="q01_wr.png">'),
    ('학습포인트', 'HTML 그대로 작성. 그림은 <img src="파일명"> 삽입 (공식·표와 교차 가능)', '선택', '<p>공식</p><img src="q01_lp.png">'),
    ('', '', '', ''),
...
def build_workbook(out_path):
    ...
    legend_rows = [
        ...
        ('해설', '오렌지', '해설', 'HTML + <img src="…"> (그림 다중·교차 OK)'),
        ('오답분석', '퍼플', '오답분석', 'HTML <ol> 권장 + <img> 가능'),
        ('학습 POINT', '에메랄드', '학습포인트', 'HTML + <img> (공식·표·그림 교차)'),
    ]
